    ...
    public void generateItemModels(ItemModelGenerators itemModelGenerator) {
        SimpleRadioItems.ITEMS.forEach((location, item) -> {
            if (item instanceof UpgradeModuleItem upgradeModuleItem) {
                itemModelGenerator.generateLayeredItem(location,
                        CommonSimpleRadio.id(upgradeModuleItem.getTier().toString().toLowerCase(), "_upgrade_module"),
                        CommonSimpleRadio.id(upgradeModuleItem.getType().toString().toLowerCase(), "_upgrade")
                );
            }
        });
    }
